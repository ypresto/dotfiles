...
begin
  ...
  $:.push(File.join(baseDir, rubyVersionArray[0..2].join("."), bitDir))
  $:.push(File.join(baseDir, rubyVersionArray[0..1].join("."), bitDir))
  # Fallback to the old architecture-free directory.
  $:.push(File.join(baseDir, rubyVersionArray[0..2].join(".")))
  $:.push(File.join(baseDir, rubyVersionArray[0..1].join(".")))
  require 'ruby-debug-base'
  
  # Update the '4' when more dirs are pushed.
  4.times { $:.pop }
  ...
  begin
    ...
    $stderr.write("  This library ships with Komodo, but also can be installed by running `gem install ruby-debug-base#{gemSuffix}'\n")
    exit
  end
end
